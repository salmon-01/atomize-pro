...
const saveTab = async (req, res) => {
  ...
  try {
    ...
    const existingTabResult = await client.query(checkIfExistsQuery, [
      name,
      icon,
    ]);
    if (existingTabResult.rows.length > 0) {
      ...
      if (existingTabResult.rows[0].icon === icon) {
        return res
          .status(400)
          .json({ message: "Tab with this icon already exists" });
      }
    }
...
const deleteTab = async (req, res) => {
  ...
  try {
    const deleteQuery =
      "DELETE FROM userinfo.tabs WHERE name = $1 RETURNING *;";
